...
/*   Updated: 2021/02/22 10:23:24 by alienard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ScavTrap::ScavTrap() : _hit_points(100), _max_hit_points(100), _energy_points(50), _max_energy_points(50),
	...
	if (PRINT)
		std::cout << "SC4V-TP Constructor called" << std::endl << "SC4V-TP "<< this->_name << " : Séquence d'initiation terminée. (noil au pet)" << std::endl;
}
...
{
	if (PRINT)
		std::cout << "SC4V-TP Destructor called" << std::endl << "SC4V-TP "<< this->_name << " : OK. Merci de m'avoir donné une deuxième chance Dieu. C'est vraiment trop sympa. (boil au nras)" << std::endl;
...
ScavTrap::ScavTrap(const ScavTrap& obj)
{
	*this = obj;
	if (PRINT)
		std::cout << "SC4V-TP Copy Constructor called" << std::endl << "SC4V-TP "<< this->_name << " : Séquence d'initiation terminée. (noil au pet)" << std::endl;
}
...
ScavTrap&	ScavTrap::operator=(const ScavTrap& obj)
{
	...
	if (PRINT)
		std::cout << "SC4V-TP Frag Assignation Operator called" << std::endl << "SC4V-TP "<< this->_name << " : Séquence d'initiation terminée. (noil au pet)" << std::endl;
	return *this;
}

unsigned int			ScavTrap::rangedAttack(std::string const & target){
	if (this->_energy_points > 0){
		std::cout << "SC4V-TP "<< this->_name << " attacks " << target << " at range, causing " << this->_range_attack_damage << " points of damage ! (without armor reduction) : TAKE THAT RUSTY MORTAL !" << std::endl;
...
unsigned int			ScavTrap::meleeAttack(std::string const & target){
	if (this->_energy_points >= 0){
		std::cout << "SC4V-TP "<< this->_name << " attacks " << target << " in melee, causing " << this->_melee_attack_damage << " points of damage ! (without armor reduction) : EAT THIS YOU BIG BABY !" << std::endl;
